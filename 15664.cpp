...
void backTracking(int* arr, int* res, bool* vis, string* str, int n, int m, int k, int st, int& j) {
	if (m == k) {
		string str2;
		for (int i = 0; i < m; i++) {
	...
	for (int i = st; i < n; i++) {
		if (!vis[i]) {
			vis[i] = true;
			res[k] = arr[i];
			backTracking(arr, res, vis, str, n, m, k + 1, i, j);
			vis[i] = false;
		}
	}
}
...
int main(void) {
	...
	sort(arr, arr + n);
	int j = 0;
	backTracking(arr, res, vis, str, n, m, 0, 0, j);
